...
def sort_string(s: str, ordering: str) -> str:
    ordering_fixed = list(dict.fromkeys(ordering))  # Elimina duplicados manteniendo el orden
    string_sorted = ''.join(c for m in ordering_fixed for c in s if c == m)
    string_sorted += ''.join(c for c in s if c not in ordering_fixed)
    return string_sorted
